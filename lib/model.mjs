// v1 already self-calibrates: lib/calibration.mjs tracks a running log-space
// bias per (model:mode) bucket from every finished run's predicted-vs-actual,
// and estimateInitial() below applies it. v2 is the bigger upgrade beyond
// that single multiplicative correction — replace this file to train on
// ~/.claude/eta/history.jsonl and do real Bayesian updating over the
// semi-Markov phases. Signatures stay identical:
//   estimateInitial(features) -> {p10,p50,p90}
//   revise(state)             -> {p10,p50,p90}

import { getBiasFactor } from "./calibration.mjs";
...
export function estimateInitial(features) {
  ...
  const p50 =
    ...
    getBiasFactor(features); // learned correction from past predicted-vs-actual misses
